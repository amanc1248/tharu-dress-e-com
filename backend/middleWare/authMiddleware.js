...
const employeeProtect = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    try {
      token = req.headers.authorization.split(" ")[1];
      const decoded = jwt.verify(token, process.env.JWT_SECRET);
      console.log("employeeToken  " + token);
      let sql =
        "select @uid :=`user_id`, first_name, last_name, email from dasa_user as var, (SELECT @uid := NULL) init_var where email=?;select @finaluid:= `user_id` from user_type, (SELECT @finaluid := NULL) init_var  where user_id =@uid AND type='employee';select employee_id from employee where user_id =@finaluid;";
      db.query(sql, [decoded.id], (err, result) => {
        if (result) {
          req.user = result;
          console.log(req.user);
...
export { protect, tailorProtect, employeeProtect, isAdmin };
